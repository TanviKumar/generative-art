...
p.draw = function() {
  p.background(255)
  for (let y = padding, i = 0; y < p.height - 2 * padding; y += cellSize, i++) {
    for (let x = padding, j = 0; x < p.width - 2 * padding; x += cellSize, j++) {
...
function drawCell(x, y, a, b) {
  ...
  p.fill(shades[1])
  p.triangle(xShift, yShift, cellSize/2, cellSize/2, cellSize/2, -cellSize/2)
  p.fill(shades[1])
  p.triangle(xShift, yShift, -cellSize/2, cellSize/2, -cellSize/2, -cellSize/2)
  p.fill(shades[3])
  p.triangle(xShift, yShift, cellSize/2, cellSize/2, -cellSize/2, cellSize/2)
