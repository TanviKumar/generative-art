...
class Branch {
  constructor () {
    ...
    this.prevx = p.mouseX;
    this.prevy = p.mouseY;
    this.color = p.color(p.random(uiColor.getValue(), uiColor.getValue() + 50), uiSaturation.getValue(), 100, uiOpacity.getValue());
  ...
  move () {
    this.direction.x += (n.simplex3(this.x * 0.03 * uiRandom.getValue(), this.y * 0.03 * uiRandom.getValue(), p.millis() * 0.0001));
    this.direction.y += p.abs((n.simplex3(this.y * 0.01, this.x * 0.01, p.millis() * 0.0001))) * 0.2;
